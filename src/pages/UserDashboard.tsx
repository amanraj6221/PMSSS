...
import socket from "@/utils/socket"; // ✅ socket config import
import {
  FileText,
  BookOpen,
  CheckCircle,
...
function UserDashboard() {
  const { applications, fetchApplications } = useApplications(); // ✅ context hook
  const navigate = useNavigate();

  // 🔹 Listen socket events for realtime updates
  useEffect(() => {
    socket.on("newApplication", () => {
      console.log("📥 New application detected");
      fetchApplications(); // refresh from backend
    });

    socket.on("sagAction", () => {
      console.log("✅ SAG action update");
      fetchApplications();
    });

    socket.on("financeAction", () => {
      console.log("💰 Finance action update");
      fetchApplications();
    });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="p-6 hover:shadow-lg transition-shadow">
          ...
          <h3 className="text-3xl font-bold text-amber-600">{sagPending}</h3>
          <p className="text-lg font-semibold mt-2 text-gray-700">Pending SAG</p>
        </Card>
        ...
        <Card className="p-6 hover:shadow-lg transition-shadow">
          ...
          <p className="text-lg font-semibold mt-2 text-gray-700">Approved</p>
        </Card>
        ...
        <Card className="p-6 hover:shadow-lg transition-shadow">
          ...
          <p className="text-lg font-semibold mt-2 text-gray-700">Disbursed</p>
        </Card>
      </div>
              ...
              >
                ...
                {app.remarks && (
                  <div className="mt-2 text-sm text-gray-600">
                    {app.remarks.sag && (
                      <p>
                        <strong>SAG:</strong> {app.remarks.sag}
                      </p>
                    )}
                    {app.remarks.finance && (
                      <p>
                        <strong>Finance:</strong> {app.remarks.finance}
                      </p>
                    )}
                    {app.remarks.payment && (
                      <p>
                        <strong>Payment:</strong> {app.remarks.payment}
                      </p>
                    )}
                  </div>
                )}
      ...
      <Card className="p-6">
        ...
        <div className="space-y-3">
          ...
          <Button
            variant="outline"
            className="w-full justify-start h-12"
            onClick={() => navigate("/dashboard/Application History")}
          >
